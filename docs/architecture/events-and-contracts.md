...
- `/domain/generation-runs/execution-state` includes a bounded `feed_attachments` map for discovered active run candidates so browser restore can rank candidates by feed truth before progress hydration: `live_attached`, then `replay_only`, then `stale`
