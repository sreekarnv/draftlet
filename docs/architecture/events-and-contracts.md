...
- runtime emits `run_started`, `variant_persisted`, `run_completed`, `run_failed`, and `run_cancelled` SSE events with bounded event ids for in-process replay; `variant_persisted` events include variant/thread/turn metadata
