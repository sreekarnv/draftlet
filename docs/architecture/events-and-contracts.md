...
- `/domain/generation-runs/execution-state` includes bounded `restore_candidates` for discovered active run candidates with run identity, lifecycle status, feed truth, replay hints, and recency/stale flags; browser restore ranks these explicit records by `live_attached`, then `replay_only`, then `stale` before progress hydration
- runtime may still emit `active`, `live`, `stale`, and `feed_attachments` on execution state as transitional compatibility data, but extension restore selection consumes `restore_candidates` and does not rebuild candidate meaning from those legacy parallel fields
