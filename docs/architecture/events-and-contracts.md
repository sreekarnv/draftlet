...
- `feed_attachments` remains on execution state as transitional compatibility data, but restore selection should consume `restore_candidates` instead of rebuilding candidate meaning from parallel `active`, `live`, `stale`, and feed maps
