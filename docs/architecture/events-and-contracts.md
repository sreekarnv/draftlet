...
- if progress hydration fails or confirms `replay_only`/`stale` instead of `live_attached`, browser restore does not claim live resume; it reconciles the run when possible and presents retry guidance for starting a fresh run from the restored thread
