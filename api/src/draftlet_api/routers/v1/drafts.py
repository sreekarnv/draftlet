...
from draftlet_api.dtos.draft import DraftCreate, DraftList, DraftRead, DraftUpdate, DraftVariantCreate, DraftVariantRead
...
async def list_drafts(conversation_id: UUID | None = None, db: AsyncSession = Depends(get_db)) -> DraftList:
...
async def create_draft(data: DraftCreate, db: AsyncSession = Depends(get_db)) -> DraftRead:
...
async def update_draft(draft_id: UUID, data: DraftUpdate, db: AsyncSession = Depends(get_db)) -> DraftRead:
    return await RuntimeService(db).update_draft(draft_id, data)


@router.post("/{draft_id}/variants", response_model=DraftVariantRead, status_code=status.HTTP_201_CREATED)
async def add_variant(draft_id: UUID, data: DraftVariantCreate, db: AsyncSession = Depends(get_db)) -> DraftVariantRead:
