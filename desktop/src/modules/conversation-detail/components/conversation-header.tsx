...
export function ConversationHeader({
  ...
  const hasAccepted = conversation.messages.some(
    (message) => message.kind === "accepted",
  );
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
            ...
            {hasDraft ? (
              <StatusBadge tone="generating">Draft available</StatusBadge>
            ) : null}
            {hasAccepted ? (
              <StatusBadge tone="ready">Inserted reply</StatusBadge>
            ) : null}
          </div>
        </div>
        ...
        <div className="flex shrink-0 flex-wrap gap-2">
          {conversation.latestDraftId ? (
            <Button size="sm" asChild>
              <Link to={`/drafts/${conversation.latestDraftId}`}>
                Open draft
              </Link>
            </Button>
          ) : (
