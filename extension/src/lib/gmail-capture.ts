...
export function extractGmailCapture(): GmailCapturePayload {
  ...
  if (!body) {
    throw new Error("Select the Gmail text you want Draftlet to capture, then try again.");
  }

  const subject = textFrom(document.querySelector("h2.hP")) || "Untitled email";
  const messageNodes = Array.from(document.querySelectorAll<HTMLElement>(".adn.ads"));
  const messageNode = selectedMessageNode(selection) || messageNodes.find((node) => isVisible(node));
  const senderNode = messageNode?.querySelector<HTMLElement>(".gD[email], .go[email]");
  const sender =
    senderNode?.getAttribute("email") || senderNode?.getAttribute("name") || textFrom(senderNode);
  const timestamp = messageNode?.querySelector(".g3")?.getAttribute("title") || undefined;
  const url = window.location.href;
  const gmailThreadId = threadIdFromUrl(url) || stableId(`${subject}:${url}`);
  const gmailMessageId = messageIdFromNode(messageNode) || stableId(`${gmailThreadId}:${body}`);
  ...
  return {
    ...
    metadata: {
      ...
      selector_strategy: messageNode ? "selection-nearest-message" : "page-fallback",
    },
  };
}
...
function selectedMessageNode(selection: Selection | null): HTMLElement | undefined {
...
function recipientsFrom(messageNode: HTMLElement | undefined): string[] {
  const nodes = Array.from(
    messageNode?.querySelectorAll<HTMLElement>(".g2[email], .gD[email], .go[email]") ?? [],
  );
  return Array.from(
    new Set(
      nodes
