...
export function extractSender(context: CaptureContext): FieldResult<string> {
  const node = firstMatching<HTMLElement>(context.messageNode, GMAIL_SELECTORS.sender);
  const sender =
    node?.getAttribute("email") || node?.getAttribute("name") || textFrom(node) || "Unknown";
...
export function extractRecipients(context: CaptureContext): FieldResult<string[]> {
  const nodes = GMAIL_SELECTORS.recipients.flatMap((selector) =>
    Array.from(context.messageNode?.querySelectorAll<HTMLElement>(selector) ?? []),
  );
...
export function extractTimestamp(context: CaptureContext): FieldResult<string | undefined> {
  const node = firstMatching<HTMLElement>(context.messageNode, GMAIL_SELECTORS.timestamp);
...
export function extractThreadId(url: string, subject: string): FieldResult<string> {
...
export function extractMessageId(context: CaptureContext, threadId: string): FieldResult<string> {
  const legacyMessageId = context.messageNode
    ?.querySelector("[data-legacy-message-id]")
    ?.getAttribute("data-legacy-message-id");
  if (legacyMessageId) return { value: legacyMessageId, source: "legacy-message-id" };

  const messageId =
    context.messageNode?.getAttribute("data-message-id") || context.messageNode?.id || undefined;
  if (messageId) return { value: messageId, source: "message-node-attr" };

  const index = context.messageNode ? context.messageNodes.indexOf(context.messageNode) : -1;
  if (index >= 0) return { value: `${threadId}:${index}`, source: "thread-message-index" };
