...
async function sendTabMessage(
	...
	try {
		...
		throw new ExtensionError(
			...
			'Open or reload Gmail, then try again.',
		);
	}
}

type UnwrapResponseArgs<T> =
	| { ok: true; result: T }
...
export async function sendExtractGmail(
	tabId: number,
): Promise<GmailCapturePayload> {
	...
	if (!result.success)
		throw new ExtensionError(
			'INVALID_MESSAGE',
			'Gmail capture returned an invalid response.',
		);
...
export async function sendCaptureGmail(
...
export async function sendInsertGmailDraft(
	tabId: number,
	text: string,
): Promise<true> {
	const response = await sendTabMessage(tabId, {
